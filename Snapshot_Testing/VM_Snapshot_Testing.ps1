<#
.SYNOPSIS
  Test and cleanup of snapshots.
.DESCRIPTION
  Using the v3 API on Prism Element, create, list, and delete snapshots the same way the backup software does. 
.PARAMETER help
...
---------- ---- ---------------------------------------------------------------
01/23/2025 ca   Initial release.
################################################################################
'@
$myvarScriptName = ".\VM_Snapshot_Testing.ps1"
...
if ($myjsonfile.ContainsKey('Logfile')) {
    ...
    $LogFile = "${PSScriptRoot}\VM_Snapshot_Testing.${LogDate}.log"
}
...
Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Get a list of VMs."
...
foreach ($vms in $myvar_resp.entities) {
    $myvar_vmname = $vms.name
    $myvar_uuid = $vms.uuid
    $myvar_vmlist.add($myvar_vmname, $myvar_uuid)
    Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Found VM $($myvar_vmname) with UUID: $($myvar_uuid)"
}

Write-Host "Select a VM for Snapshot Testing" -ForegroundColor Yellow
$myvar_vmlist.Keys | ForEach-Object { Write-Host "  $_" }
do {
    $userInput = Read-Host "Enter the VM name to use:"
} while (!($myvar_vmlist.ContainsKey($userInput)))

Write-Host "You chose: $($userInput)"
$myvar_selectedUUID = $myvar_vmlist[$userInput]
Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "User selected $($userInput) with UUID: $($myvar_selectedUUID)"

#Create the snapshot testing
$myvar_snapDate = Get-Date -Format "yyyyMMdd.HHmm"
$content = @"
{
    "spec": {
        "name":"Snapshot_Testing_$($myvar_snapDate)",
        "resources": {
            "entity_uuid":"$($myvar_selectedUUID)"
        },
...
$url = "https://$($prismelement):9440/api/nutanix/v3/vm_snapshots"
$method = "POST"
#$payload = (ConvertTo-Json $content -Depth 4) - Do not need for this variable, also payload is content
#Write-Host "$(Get-Date) [DEBUG] Payload: $($content)" -ForegroundColor White
$myvar_resp = Invoke-PrismAPICall -method $method -url $url -credential $prismCredentials -payload $content

Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Task status is: $($myvar_resp.status.state)"
$myvar_taskUUID = $myvar_resp.status.execution_context.task_uuid
$myvar_snapUUID = $myvar_resp.metadata.uuid
$url = "https://$($prismelement):9440/PrismGateway/services/rest/v2.0/tasks/$($myvar_taskUUID)"
$content = @{}
$method = "GET"
$payload = (ConvertTo-Json $content -Depth 4)
$myvar_taskPayload = Invoke-PrismAPICall -method $method -url $url -credential $prismCredentials -payload $payload
Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Snap Create Task status is: $($myvar_taskPayload.progress_status)"
while ($myvar_taskPayload.percentage_complete -lt 100) {
    Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Snap Task complete percentage is: $($myvar_taskPayload.percentage_complete)"
    Start-Sleep 1
    $myvar_taskPayload = Invoke-PrismAPICall -method $method -url $url -credential $prismCredentials -payload $payload
}
Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Snapshot Task has completed with status: $($myvar_taskPayload.progress_status)"
...
if ($myvar_resp) {
    Write-LogOutput -Category "INFO" -LogFile $LogFile -Message "Delete called with status: $($myvar_resp.status.state)"
    $myvar_delUUID = $myvar_resp.status.execution_context.task_uuid
