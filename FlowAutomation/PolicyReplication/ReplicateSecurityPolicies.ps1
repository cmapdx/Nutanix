        ...
        #region are all used address groups on the target?
            ...
            {#process each used address group
                ...
                {#based on its name, that address group does not exist on the target
                    Write-Host "$(get-date) [DATA] Missing Address Group found: $($source_address_group.address_group.name)" -ForegroundColor Red
                    Write-Host "$(get-date) [DATA] Target Address Group list: $($source_address_group.address_group.name)" -ForegroundColor Red
                    $target_address_groups.address_group.name
...
10/30/2024 ca   Removed the quarantine copy
################################################################################
'@
            ...
            {#there are rules to be updated
                ...
                {#process each rule to update
                    ...
                    #region update rule on target
                        ...
                        if (!($rule.spec.resources.quarantine_rule) -and ($rule.spec.name -ne "FSCVM-default-policy"))
                        {
